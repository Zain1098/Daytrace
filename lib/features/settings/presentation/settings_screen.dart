...
class _TrackingSettingsFormState extends ConsumerState<_TrackingSettingsForm> {
  ...
  @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(20), children: <Widget>[
    ...
    const SizedBox(height: 28), FilledButton(onPressed: end <= start || workingDays.isEmpty ? null : () async { await SettingsRepository(ref.read(appDatabaseProvider)).saveTracking(TrackingSettings(startHour: start, endHour: end, promptMinutes: prompt, workingDays: workingDays.toList()..sort())); await ref.read(todayControllerProvider.notifier).refresh(); ref.invalidate(trackingSettingsProvider); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tracking settings saved'))); }, child: const Text('Save settings')),
